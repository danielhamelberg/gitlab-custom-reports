...
class PipelineData:
    def __init__(self, gitlab_api, project_id, start_date, end_date):
        ...
        self.pipeline_ids = self.gitlab_api.list_pipeline_ids(
        ...
        for pipeline_id in self.pipeline_ids:
            variables = self.gitlab_api.get_pipeline_variables(
                self.project_id, pipeline_id)
...
# Function to instantiate a PipelineData object
def create_pipeline_data(project_id, since, until):
    ...
    # Return the PipelineData df
    return pipeline_data

# Get pipeline data
data = create_pipeline_data(
    project_id, '2021-10-01', '2022-12-31')
...
# pipelines_by_month_2021 = df_2021.groupby(
#     df_2021['date'].dt.month)['value'].sum()

# # Calculate pipelines by week in 2021
# pipelines_by_week_2021 = df_2021.groupby(
#     df_2021['date'].dt.week)['value'].sum()
...
# pipelines_by_month_2022 = df_2022.groupby(
#     df_2022['date'].dt.month)['value'].sum()

# # Calculate pipelines by week in 2022
# pipelines_by_week_2022 = df_2022.groupby(
#     df_2022['date'].dt.week)['value'].sum()
